...
export const NumberBlock: FC<{ field: AssistantField; disabled?: boolean }> = ({
  field,
  disabled = false,
}) => {
  const {
  ...
  const value =
    (state as Record<string, { value?: string | number }>)[field.key]?.value ?? field.default ?? '';
  ...
  const setNumberValue = useCallback(
    (newValue: string) => {
      setState((prev) => ({
        ...prev,
        [field.key]: { type: 'number', value: newValue },
      }));
  ...
  }, [field.key, registerFieldResetter, setNumberValue]);

  useEffect(() => {
    const timer = setTimeout(() => {
      setNumberValue(internalValue);
    }, SETUP_ASSISTANT_DEBOUNCE_MS);

    return () => {
      clearTimeout(timer);
    };
  }, [internalValue, setNumberValue]);
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <input
        ...
        placeholder={field.placeholder ?? field.default ?? 'Enter a number'}
        disabled={isDisabled}
